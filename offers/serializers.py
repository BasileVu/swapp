...
class OfferSerializer(serializers.ModelSerializer):
    def validate(self, data):
        if 'item_given' in data and self.context['request'].user.userprofile != data['item_given'].owner:
            raise serializers.ValidationError("Item given is not owned by the current user")
        if 'item_received' in data and self.context['request'].user.userprofile == data['item_received'].owner:
